...
func checkSMS(value []string) bool {
	if value[0] == CountryAlpha2()[value[0]] {
		...
		if err != nil {
			log.Printf("Значение пропускной способности канала %v не соответствует ожидаемому.", value)
			return false
		}
		if -1 < percentValue && percentValue < 101 {
			...
			if err == nil {
				...
				if value[3] == providers[value[3]] {
					...
					log.Printf("Значение провайдера %v не соответствует ожидаемому.", value)
					return false
				}
			} else {
				log.Printf("Значение ответа в ms %v не соответствует ожидаемому.", value)
				return false
			}
		} else {
			log.Printf("Значение пропускной способности канала %v не соответствует ожидаемому.", value)
		...
		log.Printf("Значение страны alpha-2 %v не соответветствует ожидаемому.", value)
		return false
	}
}

func CountryFromAlpa(str string) string {
